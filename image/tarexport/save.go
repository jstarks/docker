...
func (s *saveSession) saveImage(id image.ID) (map[layer.DiffID]*layer.ForeignSource, error) {
	img, err := s.is.Get(id)
	if err != nil {
		return nil, err
	}
	...
	var layers []string
	var foreignSrcs map[layer.DiffID]*layer.ForeignSource
	for i := range img.RootFS.DiffIDs {
		...
		ref, err := s.saveLayer(rootFS.ChainID(), v1Img, img.Created)
		if err != nil {
			return nil, err
		}
		...
		if ref != nil {
			if foreignSrcs == nil {
				foreignSrcs = make(map[layer.DiffID]*layer.ForeignSource)
			}
			foreignSrcs[img.RootFS.DiffIDs[i]] = ref
		}
	}
...
func (s *saveSession) saveLayer(id layer.ChainID, legacyImg image.V1Image, createdTime time.Time) (*layer.ForeignSource, error) {
