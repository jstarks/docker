...
func (d *Driver) importLayer(id string, layerData archive.Reader, parentLayerPaths []string) (size int64, err error) {
	cmd := reexec.Command(append([]string{"docker-windows-write-layer", filepath.Join(d.info.HomeDir, id)}, parentLayerPaths...)...)
...
func writeLayer() {
	...
	err := func() error {
		...
		w, err := hcsshim.NewLayerWriter(info, path, parentPaths)
		if err != nil {
			return err
		}
